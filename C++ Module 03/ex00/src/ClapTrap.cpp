...
void	ClapTrap::attack(const std::string& target)
{
	if (_hitPoints <= 0 || _energyPoints <= 0)
		std::cout << GREEN "ClapTrap " << _name << " has not enough energy or hit points to attack" RESET << std::endl;
	else
	{
		_energyPoints--;
...
void	ClapTrap::takeDamage(unsigned int amount)
{
	if (_hitPoints > 0)
	{
		...
		std::cout << GREEN "ClapTrap " << _name << " take " << amount << " points of damage" RESET << std::endl;
	}
	else
		std::cout << GREEN "ClapTrap " << _name << " is already disabled" RESET << std::endl;
}

void	ClapTrap::beRepaired(unsigned int amount)
{
	if (_hitPoints <= 0 || _energyPoints <= 0)
		std::cout << GREEN "ClapTrap " << _name << " has not enough energy or hit points to repair" RESET << std::endl;
	else
	{
		_hitPoints += amount;
		_energyPoints--;
